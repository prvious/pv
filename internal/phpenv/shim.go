...
// WriteShims creates the php, frankenphp, and composer shim scripts in ~/.pv/bin/.
func WriteShims() error {
	...
	composerContent := fmt.Sprintf(composerShimScript,
		config.ComposerDir(),
	...
	if err := os.WriteFile(composerShim, []byte(composerContent), 0755); err != nil {
