// Package mailpit holds cobra commands for the mailpit:* / mail:* group.
// mailpit is the canonical command name (matching the binary); mail:*
// is a hidden alias preserving the user-facing service name from when
// this service lived under service:add mail.
package mailpit
...
// Register wires every mailpit:* command + the hidden mail:* alias variant onto parent.
func Register(parent *cobra.Command) {
	...
	for _, c := range cmds {
		parent.AddCommand(c)
		parent.AddCommand(aliasCommand(c, "mailpit:", "mail:"))
	}
}
