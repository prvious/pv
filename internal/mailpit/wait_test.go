...
func TestWaitStopped_AlreadyDown(t *testing.T) {
	// Port 1025 should be down in test env; WaitStopped returns immediately.
...
func TestWaitStopped_TimeoutWhenUp(t *testing.T) {
	// Bind a listener on the Mailpit port so WaitStopped times out.
	ln, err := net.Listen("tcp", "127.0.0.1:1025")
	if err != nil {
		t.Skipf("cannot bind port 1025: %v", err)
	}
	defer ln.Close()
