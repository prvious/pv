...
Before creating candidate files, PV deterministically renders each desired active-form root and its fragments in memory. PV fingerprints a scheme version, the exact root bytes, and each exact fragment filename and content in filename order with explicit length framing. A successfully applied fingerprint is stored separately from process-spec identity. For an ordinary reconciliation whose owned process identity, admin endpoint, replacement marker, and applied fingerprint all match, PV re-verifies ownership, performs one cheap readiness probe, and skips candidate creation, validation, promotion, backups, and `POST /load`. A failed fast-path readiness probe falls through once to the full transaction. Caddy `GET /config/` output and generated files on disk are never config-equality inputs; changing PV's application version alone does not change the fingerprint. A future forced-restart operation must bypass this no-op path.
