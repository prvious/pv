...
Readiness probe failures and bounded probe timeouts retain their error details in the existing structured runtime-health diagnostics. The scanner does not write those diagnostics to `pv.db`.
...
Before creating candidate files, PV deterministically renders each desired active-form root and its fragments in memory. PV fingerprints a scheme version, the exact root bytes, and each exact fragment filename and content in filename order with explicit length framing. A successfully applied fingerprint is stored separately from process-spec identity. For an ordinary reconciliation whose owned process identity, admin endpoint, replacement marker, and applied fingerprint all match, PV re-verifies ownership, performs one cheap readiness probe, and skips candidate creation, validation, promotion, backups, and `POST /load`. A failed fast-path readiness probe falls through once to the full transaction. Caddy `GET /config/` output is never a config-equality input. Generated files on disk are checked for drift and hardening after the in-memory proof matches, but do not establish that proof. On the byte-identical fast path, PV validates ownership and restores the generated root and desired fragments to `0600` and their directories to `0700` without changing bytes, validating or promoting config, reloading Caddy, or restarting the process; unexpected ownership fails reconciliation visibly. Changing PV's application version alone does not change the fingerprint. A future forced-restart operation must bypass this no-op path.

Generated Gateway and worker files are disposable output, so editing a valid Project's generated root or fragment does not trigger a reload; the next full application overwrites the edit. As a current exception, an invalid Project preserves its last-valid active fragment and uses those bytes as desired-render input, so editing that preserved fragment changes the next desired fingerprint. Out-of-band config loads and `SIGUSR1` reloads from edited generated files are unsupported and can invalidate the recorded application proof.

When no matching PV-owned process exists, PV validates the candidate with the correct managed binary, promotes the active config, starts the process, and immediately records the promoted fingerprint as staged while marking the runtime replacement-required. A staged fingerprint proves the exact prepared bytes, not application or readiness, and may survive process death when a retained source route still needs those bytes for a safe retry. After admin readiness through `GET /config/`, PV verifies the existing Gateway or worker readiness check and atomically moves the staged fingerprint to the applied fingerprint while clearing the marker. Under the existing `PreserveRuntime` policy, a process that remains alive and owned after public readiness fails may instead commit the fingerprint and report degraded, unverified routing. A process replacement is allowed when the process is absent or its recorded runtime identity/spec is obsolete; it is not a reload fallback.

After either verified readiness or the `PreserveRuntime` commit outcome, deleting the previous root and fragment backups is post-commit housekeeping. PV attempts both deletions independently. A cleanup failure leaves the promoted config and committed runtime in place, emits one structured warning containing every failed backup path, and does not trigger rollback, a compensating load, or a process stop.

When a matching PV-owned process exists, PV verifies ownership immediately before the admin request. Before sending `POST /load`, PV atomically records the desired fingerprint as staged, marks that exact runtime metadata as replacement-required, and clears its applied fingerprint. The marker remains set through post-load readiness. Accepted application with verified readiness, or accepted application under the existing `PreserveRuntime` policy with admin readiness and ownership still proven, atomically moves the staged fingerprint to the applied fingerprint and clears the marker. A rejected or unavailable `POST /load` relies on Caddy's atomic load contract to keep the previous in-memory config active. PV verifies the previous generated root and fragments against the previous applied fingerprint, restores them on disk, and restores that fingerprint while clearing the marker, keeps the owned process and PID running, records the typed failure, and never signals or restarts that process as a fallback. Runtime metadata retains the newer desired fingerprint after rollback so the health tick can retry unapplied config without rebuilding a runtime plan. If no previous applied fingerprint exists, PV clears the marker without claiming application. Rollback or final metadata-write failure leaves the marker set, the applied fingerprint absent, and the staged fingerprint available when it was recorded successfully.

When the client cannot determine whether a `POST /load` is still executing or was accepted, PV does not issue a competing compensating load because the final request ordering cannot be established. PV keeps the promoted desired root and fragments on disk, preserves the owned process, leaves its replacement-required marker and staged fingerprint set, and records the typed unknown-outcome failure without claiming which config is active. A later reconciliation never sends another load to that process: it stops the verified marked process, waits for its process group and pending handlers to exit, then starts a replacement from the latest desired config. The replacement establishes a new process epoch before reconciliation can report success, so an older unresolved load cannot overtake newer desired state.

After a successful load, PV runs the existing runtime readiness checks. If a real post-load readiness failure requires rollback, PV verifies that the backup tree still hashes to the previously recorded applied fingerprint, restores the previous files, re-verifies ownership of the same process, loads the restored previous root through the same admin endpoint, and verifies the previous readiness before restoring that fingerprint. PV returns the original failure after a successful rollback. If the backup fingerprint differs, PV restores disk state but does not load the possibly tampered backup. If restored-config load or readiness fails, PV returns a compound failure containing both errors and does not claim that the prior runtime was restored. If an ownership check immediately after a successful load or after readiness succeeds fails, PV keeps the promoted desired files without a compensating load. On the rollback path, PV restores disk state before re-verifying ownership and does not contact an unverified process. Every unproven outcome keeps the replacement marker set and the applied fingerprint absent. A PF `drifted` or `unknown` result handled by the existing `PreserveRuntime` policy is recorded as degraded routing state and does not by itself roll back a Caddy-accepted config.
...
`pv update --check` is read-only and does not retain either lock for the duration of the check. At its CLI and daemon preflight boundaries, it fails clearly if a foreground self-update owns `update.lock` instead of waiting during the daemon transition.
...
The two locks cover the network fetch, binary swap, and daemon transition. `pv update` does not enqueue `reconcile system` or `update system` while it holds them. The Managed Resource update job is submitted only after the foreground app phase releases both locks or after the re-execed continuation starts.
...
PV uses two OS-level advisory filesystem locks. `~/.pv/run/update.lock` protects foreground self-update and the daemon transition. `~/.pv/run/jobs.lock` protects the daemon mutation queue. `pv update` acquires `update.lock` and then `jobs.lock` before changing the application or daemon; the daemon queue acquires only `jobs.lock` and never waits while holding `update.lock`. Ordinary mutating CLI preflight checks only `update.lock`, so an active daemon reconciliation does not reject unrelated intent recording. Direct foreground Managed Resource install, update, and PHP selection commands also hold `jobs.lock` through artifact and related selection mutations, then release it before submitting daemon reconciliation. Once startup reconciliation begins running, daemon shutdown waits for it to finish so blocking installation work retains the jobs lock through completion. Startup waiting for the jobs lock remains cancellable.
...
Reconciliation scopes are `system`, `project:<id>`, and `resource:<name>:<track>`. Whole-system setup/update uses `system`; Project config changes use `project:<id>`; explicit Managed Resource install/update work uses `resource:<name>:<track>`. If dependencies overlap in a way that is hard to isolate safely, the daemon may promote work to `system` scope. Project-to-System promotion is fail-closed: only explicit ownership or scope-proof uncertainty — an unprovable active process, fingerprint, or persisted identity — promotes. Invalid configuration, database, permission, filesystem, and substantive runtime errors remain visible and fatal instead of promoting.

Reconciliation records elapsed time per phase in the structured daemon log. Top-level phase timers run sequentially and never overlap one another, so their elapsed times sum within the job's total execution time. Resources additionally suppresses the per-operation records that its artifact work would otherwise time inside it, so that work is counted once by the phase that owns it. Artifact downloads still report live progress to the waiting command while Resources is open, but that progress is streamed rather than recorded as its own timed phase.

Project-scoped reconciliation applies a Project in two stages around the Resources phase. The first Project Apply validates Project config and records every Managed Resource the Project needs — its declared backing resources and its PHP/FrankenPHP pair. It may read cached artifact metadata to resolve a default or `latest` track, but it downloads and installs nothing; a failure there ends the job before Resources runs. Resources then installs the Project's declared installable backing-resource tracks, and owns the download progress reporting for that work; other desired artifacts are installed only by the conditional repair pass described below, under its gate. The final Project Apply reconciles the Project's resources and allocations, starts what the Project needs, and renders the Project environment.

A Project scope installs the installable Managed Resource tracks that Project declares, meaning those PV has a runtime adapter for. A declared track with no adapter has no artifact to install, so it is left to Project Apply, which tolerates it when its track env context is already seeded and otherwise records the failure. The Project scope then may run a system-wide repair pass, gated on the Project requesting optional PHP extensions and any desired Gateway runtime track being missing. That pass installs every desired track that is missing, so an unrelated missing artifact can still be installed by it and can still fail the Project's Resources phase. A `resource:<name>:<track>` scope for a backing service installs the named track's install set, while `caddy`, `php`, and `frankenphp` resource scopes reconcile the Gateway runtime instead of installing artifacts. Resources is reported as skipped when the scope needs no install work.

Whole-system reconciliation replaces the first Project Apply with a read-only Demand Discovery pass, so it normally proceeds through Demand Discovery, Resources, then Project Apply. Demand Discovery resolves every linked Project's demand without writing state; a Project whose demand cannot be resolved falls back to its last applied demand. Resources installs the discovered demand, retrying the install once before the apply when the first attempt fails, then discovers demand from Projects linked while it was running and installs anything they newly require. The final Project Apply is staged and cannot install: it reports a missing artifact instead of fetching it. The apply records per-Project failures instead of stopping, continuing with each linked Project.

A `resource:<name>:<track>` scope stages the same way around its own Resources phase: it records requirements, installs the scope's track when a runtime adapter provides it, reconciles the track's runtime and allocations, then applies the dependent Projects' environments. A dependent Project whose recorded demands no longer match its config, whose PHP identity changed, or whose established backing resource is unready keeps its failure and is skipped by the apply rather than rendered from a runtime another scope owns.

Because artifact download and installation belong to Resources, a failure while locating, downloading, or installing an artifact in that phase is attributed to Resources rather than to Project Apply. Resolving a default or `latest` track from cached artifact metadata happens in the first Project Apply, so a failure there is a Project Apply failure; otherwise a Project Apply failure means the Project's own configuration, resource application, or environment rendering failed. Project-scoped reconciliation reports Resources as skipped when no in-scope resource work is needed; whole-system reconciliation reports that pass as succeeded whether or not it had anything to install.
...
Postgres import maps a source database with a declared allocation to that allocation's persisted `generated_name`. For an undeclared user database, it applies the normal allocation-name validation and naming rule to create a Project-namespaced physical database, but does not change `pv.yml` or add an allocation record to `pv.db`. The plan warns that the database is unmanaged and shows the allocation snippet that would let later reconciliation adopt it. A single-database dump without a discoverable source name requires an explicit target allocation.

Postgres import accepts multi-database `pg_dumpall` scripts. Its plan explicitly lists cluster-global roles, memberships, role passwords, tablespaces, and cluster configuration that it skips. It never remaps or restores `template0` or `template1`, and skips contents of the `postgres` maintenance database unless the user explicitly maps that source database. Dump SQL runs under a temporary restricted PostgreSQL login role, never under `pv_root`. During execution, temporary PV-owned `pg_hba.conf` rules permit that role to connect only to mapped targets and reject it on other databases over TCP and local sockets. PV verifies the rules before executing dump SQL and removes the role and rules after import. A preflight or isolation setup failure executes no dump SQL.

Postgres import creates target databases with PV's provider defaults. The plan lists source database encoding, locale, collation, template, and database-level settings that it omits. It rejects an incompatible source encoding before execution. It omits source object ownership and ACLs, lists those changes in the plan, and assigns imported objects to `pv_root` after execution.

Postgres import permits ordinary functions and row triggers that pass preflight. It rejects `SECURITY DEFINER` functions and procedures: reassigning their owner to the track-wide `pv_root` superuser would increase their execution privileges. It also rejects subscriptions and event triggers. Table-scoped publications are allowed only for imported tables; publications for all tables or entire schemas are rejected. It may install extensions supplied by the selected Postgres track only when PostgreSQL marks them trusted; unavailable or privileged extensions fail preflight. Standard `pg_dump` large objects may be imported when their OIDs do not conflict with target objects; an unresolved conflict fails preflight.
