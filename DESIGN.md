...
When no matching PV-owned process exists, PV validates the candidate with the correct managed binary, promotes the active config, and starts the process without an applied fingerprint. After admin readiness through `GET /config/`, PV marks the fresh runtime replacement-required until it verifies the existing Gateway or worker readiness check and atomically records the fingerprint. Under the existing `PreserveRuntime` policy, a process that remains alive and owned after public readiness fails may instead commit the fingerprint and report degraded, unverified routing. A process replacement is allowed when the process is absent or its recorded runtime identity/spec is obsolete; it is not a reload fallback.
...
When a matching PV-owned process exists, PV verifies ownership immediately before the admin request. Before sending `POST /load`, PV atomically marks that exact runtime metadata as replacement-required and clears its applied fingerprint. The marker remains set through post-load readiness. Accepted application with verified readiness, or accepted application under the existing `PreserveRuntime` policy with admin readiness and ownership still proven, atomically records the new fingerprint and clears the marker. A rejected or unavailable `POST /load` relies on Caddy's atomic load contract to keep the previous in-memory config active. PV restores the previous generated root and fragments on disk before clearing the marker while leaving the fingerprint absent, keeps the owned process and PID running, records the typed failure, and never signals or restarts that process as a fallback. Rollback or final metadata-write failure leaves the marker set and the fingerprint absent.
