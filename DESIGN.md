...
PV uses two OS-level advisory filesystem locks. `~/.pv/run/update.lock` protects foreground self-update and the daemon transition. `~/.pv/run/jobs.lock` protects the daemon mutation queue. `pv update` acquires `update.lock` and then `jobs.lock` before changing the application or daemon; the daemon queue acquires only `jobs.lock` and never waits while holding `update.lock`. Ordinary mutating CLI preflight checks only `update.lock`, so an active daemon reconciliation does not reject unrelated intent recording. Once startup reconciliation begins running, daemon shutdown waits for it to finish so blocking installation work retains the jobs lock through completion. Startup waiting for the jobs lock remains cancellable.
