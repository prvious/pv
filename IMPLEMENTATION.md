...
| PV-063 | Implement Project config parser                    | Enabler | PV-060         | env rendering, Managed Resource needs | `pv.yml`/`pv.yaml`, strict keys after YAML anchor/alias/merge resolution, document root, hostnames, Managed Resources, Resource allocations, and env maps validate. |
...
| PV-112 | Add PHP/FrankenPHP artifact recipes                        | Story   | PV-110                                         | M7 public artifacts             | Recipes build PHP `8.3`, `8.4`, and `8.5` plus matched FrankenPHP artifacts, enforce the fixed extension set, validate PHP/FrankenPHP patch-version sync, and smoke-test CLI plus loopback serving.         |
...
| PV-117 | Publish initial Managed Resource artifact matrix           | Task    | PV-111, PV-112, PV-113, PV-114, PV-115, PV-116 | release candidate               | The staging RC manifest contains the default setup install set for `darwin-arm64`; full public v1 expands native validation to `darwin-arm64,darwin-amd64` when the conditional Intel gate is cleared.      |
...
| PV-119 | Add PV app release and publication workflows               | Enabler | PV-106, PV-107                                 | public PV app distribution      | GitHub Actions builds native PV app binaries, writes app release records, generates the stable app manifest and installer, stages app publication, and uploads immutable and stable app objects to R2.      |
...
| PV-125 | Release candidate checklist | Task | PV-120, PV-121, PV-122, PV-123, PV-124 | public v1 | `docs/release/rc-checklist.md` confirms install, setup, link, Managed Resources, update, uninstall, artifact publication, and diagnostics.                    |
...
| PR 12A | Platform and protocol workspace boundary refactor                                                                | PV-005                                                    | PR 10, PR 11, PR 12                                 | No                                        | Yes (#253)                               |
| PR 13  | LaunchAgent, `pv setup`, safe `pv uninstall`, and System keychain CA trust/untrust mutation                      | PV-054 (System keychain mutation), PV-055, PV-056, PV-057 | PR 7, PR 10, PR 11, PR 12, PR 12A                   | No                                        | Yes (#254)                               |
| PR 14  | PHP/FrankenPHP adapters, workers, Gateway, first Project-serving test                                            | PV-070, PV-071, PV-072, PV-073, PV-074, PV-078            | PR 7, PR 9, PR 13                                   | No                                        | Yes (#256)                               |
| PR 15  | PHP shim, PHP commands, Composer track `2` install/shim                                                          | PV-075, PV-076, PV-077                                    | PR 14                                               | Yes                                       | Yes (#258)                               |
| PR 16  | Mailpit adapter                                                                                                  | PV-083                                                    | PR 7, PR 9                                          | Yes                                       | Yes (#272)                               |
| PR 17  | Redis adapter                                                                                                    | PV-082                                                    | PR 7, PR 9                                          | Yes                                       | Yes (#273)                               |
| PR 18  | MySQL adapter                                                                                                    | PV-080                                                    | PR 7, PR 9                                          | Yes                                       | Yes (#275)                               |
...
| PR 22A | Staging/production artifact manifest endpoint selection for CLI and daemon                                       | PV-118                                                    | PR 6B, PR 13, PR 21, PR 25                          | No, unlocks staging dogfood               | Yes (#280)                               |
| PR 22B | PV app update manifest parser, self-update release layout, and update filesystem lock                            | PV-100, PV-101, PV-102                                    | PR 22A, PR 13                                       | No                                        | Yes (#281)                               |
| PR 22C | `pv update --check` for PV app and installed Managed Resource update availability, including JSON                | PV-104                                                    | PR 22B, PR 7, PR 13, and at least one real adapter  | Yes, after PR 22B                         | Yes (#282)                               |
| PR 22D | PV application self-update, daemon restart coordination, migration health check, and rollback                    | PV-103                                                    | PR 22B, PR 13                                       | No                                        | Yes (#283)                               |
| PR 22E | Managed Resource update orchestration for installed tracks after the PV application update phase                 | PV-105                                                    | PR 22C, PR 22D, PR 7, and at least one real adapter | No                                        | Yes (#284)                               |
| PR 22F | Generated installer script contract, PV app release metadata, and app release/publication workflows              | PV-106, PV-107, PV-119                                    | PR 22B, PR 13                                       | Yes, after PR 22B                         | Yes (#285)                               |
...
- PV app release workflow publishes immutable app binaries, app records, stable `pv-app-manifest.json`, and stable `install.sh`.
...
- Default PHP/FrankenPHP `8.5` and Composer `2` artifacts are published for the Apple Silicon/staging RC target.
- Default MySQL `8.4`, Postgres `18`, Redis `8.8`, Mailpit `1`, and RustFS `1` artifacts are published for the Apple Silicon/staging RC target.
- Full public v1 readiness expands native Managed Resource validation to `darwin-arm64,darwin-amd64`; until the conditional `darwin-amd64` gate is cleared, the RC must explicitly document the Apple Silicon/staging scope and any intentionally missing Intel resource/platform evidence.
