...
- Create: `crates/pv-release/tests/smoke.rs`
  - Smoke hook coverage.
...
Expected: FAIL because smoke behavior is not implemented.

- [ ] **Step 3: Implement smoke hook runner**
...
- [ ] **Step 4: Run smoke tests and accept snapshots**

Run:

```shell
cargo insta test --accept --test-runner nextest -p pv-release --test smoke
```
...
```shell
git add crates/pv-release/src/smoke.rs crates/pv-release/tests/smoke.rs crates/pv-release/tests/snapshots
git commit -m "feat(release): validate smoke hooks"
```
