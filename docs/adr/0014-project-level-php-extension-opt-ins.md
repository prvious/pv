...
PV will keep the default loaded PHP extension set lean but Laravel-practical. `ftp` and `gd` are default-loaded extensions, with GD built against FreeType, JPEG, AVIF, and WebP. The initial optional catalog is `redis`, `sqlsrv`, `pdo_sqlsrv`, `xdebug`, `apcu`, `pcov`, `imagick`, `mongodb`, `yaml`, and `rar`. Future extensions should be added only when user demand justifies the build, smoke-test, license, and support burden.
