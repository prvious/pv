...
func Uninstall(version string, force bool) error {
	...
	if err := os.RemoveAll(config.MailpitVersionDir(version)); err != nil {
		return fmt.Errorf("remove mailpit version dir: %w", err)
	}
	...
	if vs, err := binaries.LoadVersions(); err == nil {
		delete(vs.Versions, "mailpit-"+version)
		if err := vs.Save(); err != nil {
			return fmt.Errorf("save versions state: %w", err)
		}
	}
	if force {
		if err := os.RemoveAll(config.MailpitDataDir(version)); err != nil {
